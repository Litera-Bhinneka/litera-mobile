...
class _RecommendationDialogState extends State<RecommendationDialog> {
  ...
  Future<List<String>> fetchUserItem() async {
    var url = Uri.parse(
        'https://litera-b06-tk.pbp.cs.ui.ac.id/recommendation/get-user-inventory$username');
    ...
    List<String> items =
        List<String>.from(data['book_titles'].map((dynamic item) {
      if (item != null) {
            ...
            child: Column(
              ...
              children: [
                ...
                DropdownButtonFormField<String>(
                  ...
                  onChanged: dropdownItems.length < 2
                      ? null
                      : (String? value) {
                          setState(() {
                            _book_title = value!;
                  ...
                  decoration: InputDecoration(
                    labelText: dropdownItems.length < 2
                        ? 'You must atleast have 2 books'
                        : 'Select Book Title',
                  ),
                  isExpanded: true,
                ),
                          ...
                          onPressed: () async {
                            if (_formKey.currentState!.validate()) {
                              final http.Response response = await http.post(
                                Uri.parse(
                                    "https://litera-b06-tk.pbp.cs.ui.ac.id/recommendation/create-flutter/"),
                              ...
                              if (response.statusCode == 200) {
                                ...
                                if (responseBody['status'] == 'success') {
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(
                                      content: Text(
                                          "Recommendation berhasil disimpan!"),
                                    ),
                                  );
